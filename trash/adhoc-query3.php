...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    foreach ($tables as $table) {
        ...
        $joinClause .= $table . '.customer_id = service.customer_id OR ';
        $joinClause .= $table . '.employee_id = service.employee_id OR ';
        $joinClause .= $table . '.receipt_id = service.receipt_id OR ';
        $joinClause .= $table . '.vehicle_id = service.vehicle_id OR ';
        $joinClause .= $table . '.column = service.column ';
    }
    
    $tablePart = rtrim($tablePart, ', ');
...
<body>
    <form action="" method="post" onsubmit="return validateForm()">
        <label for="table">Table(s):</label>
        <div id="table-container">
            <input type="text" name="tables[]" value="service" readonly>
            <input type="text" name="tables[]" placeholder="Table">
            <input type="text" name="tables[]" placeholder="Table">

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($customerName) || empty($email) || empty($password) || empty($phone) || empty($model) || empty($colour) || empty($plate) || empty($price) || empty($package) || empty($appointmentDate)) {
        ...
        if (oci_fetch_assoc($statement)) {
            ...
            $insertReceiptQuery = "INSERT INTO receipt (receipt_bill, receipt_date) VALUES (:price, SYSDATE)";
            ...
            if (oci_execute($insertCustomerStatement) && oci_execute($insertVehicleStatement) && oci_execute($insertReceiptStatement)) {
                ...
                $receiptQuery = "SELECT receipt_id FROM receipt WHERE receipt_bill = :price";
                $receiptStatement = oci_parse($connection, $receiptQuery);
                oci_bind_by_name($receiptStatement, ':price', $price);
                oci_execute($receiptStatement);
                $receiptRow = oci_fetch_assoc($receiptStatement);
                $receiptId = $receiptRow['RECEIPT_ID'];

                // Retrieve an employee ID
                $employeeQuery = "SELECT employee_id FROM employee ORDER BY DBMS_RANDOM.VALUE FETCH FIRST 1 ROWS ONLY";
                ...
                $error = 'Error occurred during customer, vehicle, and receipt data insertion. Please try again.';
            }
        }
    }
}
